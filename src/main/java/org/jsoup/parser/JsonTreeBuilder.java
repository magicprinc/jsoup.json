...
/**
 * Powerful JSON (<a href="http://www.ietf.org/rfc/rfc4627.txt">RFC 4627</a>, 7158, <a
 * href="http://www.ietf.org/rfc/rfc7159.txt">RFC 7159</a>) parser. It can consume almost every text
 * as JSON and convert it internally to an XML-tree. <br>
 * <br>
 * Heavily inspired by Google Gson (c) 2010 Google Inc. <br>
 * <br>
 * [example.json]
 * <pre>
 * {projects = [
 ...
 * </pre>
 *
 * @author Andrej Fink [dev@deltaf.one]
 * @author Jesse Wilson
...
public class JsonTreeBuilder extends XmlTreeBuilder {
  ...
  /**
   * Extension point: one can add "number", "string", "url", etc. `class` detection.<br>
   * Default implementation: if argument extraInfoInAttrs of {@link #jsonParser(boolean)} is true -
   * add detected "class" of value as attribute "class", e.g. class="quot quoted num".
   *
   * @param el current {@link Element} (optional json key can be found in "id" attribute)
   * @param value current json value
   * @param typeClass value is {@link VALUE_CLASS#SINGLE_QUOTED}, {@link VALUE_CLASS#DOUBLE_QUOTED}
   *     or {@link VALUE_CLASS#UNQUOTED}
   */
  ...
  /**
   * Create a new JSON parser.<br>
   * This parser reads almost every text as JSON and creates a simple [xml] tree.<br>
   *
   * @param extraInfoInAttrs add extra metadata (class attribute with detected class) to value Elements
   ...
   * This parser reads almost every text as JSON and creates a simple [xml] tree.<br>
   * <br>
   * Same as {@link #jsonParser(boolean)}(extraInfoInAttrs = true)
   * <br><br>
   * <pre><code>
  ...
  /**
   * Converts JSON String to XML String. <br>
   * (Auxiliary utilitarian method)
   *
